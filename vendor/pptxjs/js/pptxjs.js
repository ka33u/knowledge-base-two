...
(function ($) {
    $.fn.pptxToHtml = function (options) {
        ...
        function failPreview(error) {
            console.error(error || "PPTX preview failed");
            $result.empty().append($("<p></p>").addClass("pdf-note").text("PPTX 预览转换失败，原文件仍保存在服务器中。"));
            $result.trigger("pptx:error", [error]);
        }
        ...
        if (settings.pptxFileUrl != "") {
            try{
                JSZipUtils.getBinaryContent(settings.pptxFileUrl, function (err, content) {
                    if (err || !content) {
                        failPreview(err || new Error("PPTX file is empty"));
                        return;
                    }
        ...
        function convertToHtml(file) {
            ...
            if (file.byteLength < 10){
                failPreview(new Error("PPTX file is empty"));
                return;
            }
            var zip = new JSZip(), s;
        ...
        */
        function processPicNode(node, warpObj, source, sType) {
            ...
            var blipAttrs = getTextByPathList(node, ["p:blipFill", "a:blip", "attrs"]) || {};
            var rid = blipAttrs["r:embed"] || blipAttrs["r:link"];
            if (!rid) {
                console.warn("PPTXjs: skipped picture without a relationship id");
                return "";
            }
            var resObj;
            ...
            var relationship = resObj && resObj[rid];
            if (!relationship || !relationship["target"]) {
                console.warn("PPTXjs: skipped picture with a missing relationship", rid);
                return "";
            }
            var imgName = relationship["target"];

            //console.log("processPicNode imgName:", imgName);
            var imgFileExt = extractFileExtension(imgName).toLowerCase();
            var zip = warpObj["zip"];
            var imageFile = zip.file(imgName);
            if (!imageFile) {
                console.warn("PPTXjs: skipped unavailable picture", imgName);
                return "";
            }
            var imgArrayBuffer = imageFile.asArrayBuffer();
            var mimeType = "";
